...
| `agent.notifications.sound.enabled`              | `true`                                                                           | window   | 会話が停止したときと、承認待ち・質問で止まったときに、それぞれ別の音を鳴らす（Codex/Claude Code両画面共通）。無効にすると一切鳴らさない                                                                                                                     |
...
| `agent.notifications.sound.playerCommand`        | `""`                                                                             | window   | 音を鳴らす外部コマンドの上書き。`${file}` を音源のパスへ置換する。空なら環境から自動で探す（Linux: `paplay` / `pw-play` / `aplay` / `ffplay`、macOS: `afplay`、Windows: `powershell.exe`）。シェルは介さない                                                |
