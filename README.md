...
**タブが復元されない**
チャット画面はVSCodeのWebview復元に任せているため、VSCodeがタブを復元しない設定になっていると戻らない。Claude Codeのforkで作ったセッションは、CLIが振る新しいidを追えないため復元対象外になる。
...
- **復元は会話履歴ベース**: 中断したターンの応答は戻らない（Codexが保存しないため）
...
| 領域                                                     | 状態                                                   |
| -------------------------------------------------------- | ------------------------------------------------------ |
| 純粋ロジック層（引数組み立て・パーサ・一覧・状態遷移）   | 完了、テスト434件                                      |
...
| 履歴・タブ復元                                           | Codex側を実機確認済み。TUI廃止後の回帰は未確認         |
| 作業記録の日報連携                                       | 実装完了、収集スクリプトとの疎通を確認済み             |
| パッケージング（vsix生成）                               | 完了                                                   |

実VSCodeでしか確認できない範囲のチェックリストは [docs/manual-test.md](docs/manual-test.md)、実施記録は [docs/manual-test-2026-08-10.md](docs/manual-test-2026-08-10.md) にある。
