...
役割を分けると、メッセージ1本には収まらない情報（設計メモ、レビュー結果、共有コンテキスト）を渡したくなる。そのための一時的な置き場が `.agents/handoff/runs/<runId>/<taskId>-<スラッグ>.md` で、MCPツール `write_handoff` / `read_handoff` / `list_handoffs` / `delete_handoff` で読み書きする。オーケストレーター・タスクの双方から使える。
