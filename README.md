...
- Claude Code: 一覧からskillを隠し、選んだskillを`/skill名 <発言>`の形で送る。隠したskillはモデルがSkillツールで呼んでも拒否されるため、有効にした会話ではモデルが自分からskillを使うことはなくなる。自分で`/skill名`と打てば使える。`/`で呼べないskill（`user-invocable: false`）は隠さず、選択の候補にもしない
...
- 途中で無効にしても、Codexでは有効な間に始めた会話の一覧は外れたまま、Claude Codeで隠したskillはそのセッションのCLIプロセスが終わるまで隠れたまま
