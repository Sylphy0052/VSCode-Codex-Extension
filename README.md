...
| `agent.notifications.os.enabled`                 | `false`                                                                          | window   | WindowsのOS通知（トースト）を出すか。WSLまたはWindowsで `powershell.exe` が使える場合にだけ効く。無効の間は `powershell.exe` を一切起動しない。クリックで該当タブを最前面に出すには、Windows側で `Install-Module BurntToast -Scope CurrentUser` が必要（未導入でも通知は出るが、クリックしても何も起きない）                     |
| `agent.notifications.os.turnComplete`            | `true`                                                                           | window   | 会話が停止した（ターンが完了した）ときにOS通知を出すか                                                                                                                                                                                                                                                                           |
| `agent.notifications.os.approvalPending`         | `true`                                                                           | window   | 承認待ち・質問で止まったときにOS通知を出すか。1ターンで複数の承認要求が同時に出ても通知は1回だけ                                                                                                                                                                                                                                 |
| `agent.notifications.os.handoff`                 | `true`                                                                           | window   | 自動引き継ぎが発火したときにOS通知を出すか。手動の引き継ぎボタンでは出さない                                                                                                                                                                                                                                                     |
| `agent.notifications.os.onlyWhenHidden`          | `false`                                                                          | window   | そのタブが見えているときはOS通知を出さない。既定は無効（通知音の同名設定と同じ考え方）                                                                                                                                                                                                                                           |
| `agent.notifications.sound.enabled`              | `true`                                                                           | window   | 会話が停止したとき、承認待ち・質問で止まったとき、自動引き継ぎが起きたときに、それぞれ別の音を鳴らす（Codex/Claude Code両画面共通）。無効にすると一切鳴らさない                                                                                                                                                                  |
| `agent.notifications.sound.turnComplete`         | `true`                                                                           | window   | 会話が停止した（ターンが完了した）ときに音を鳴らすか。既定の音は `resources/se_sac03.wav`                                                                                                                                                                                                                                        |
...
| `agent.notifications.sound.handoff`              | `true`                                                                           | window   | 自動引き継ぎが発火したときに音を鳴らすか。既定の音は `resources/se_sad03.wav`。手動の引き継ぎボタンでは鳴らさない                                                                                                                                                                                                                |
...
| `agent.notifications.sound.handoffFile`          | `""`                                                                             | window   | 自動引き継ぎが起きたときに鳴らすWAVの絶対パス。空なら同梱音源                                                                                                                                                                                                                                                                    |
