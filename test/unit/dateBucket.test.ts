...
describe('dateBucketFor（issue #293、design.md §14.54）', () => {
  it('同じ暦日は今日', () => {
    expect(dateBucketFor(iso(2026, 8, 16, 0, 0), NOW)).toBe('today');
  ...
  it('日付をまたぐ境界（23:59→翌0:00）で「今日」から「昨日」に変わる', () => {
    // 「今」が0:30のとき、23分前（前日23:59）はもう昨日
    const midnightNow = new Date(2026, 7, 16, 0, 30).getTime();
    expect(dateBucketFor(iso(2026, 8, 15, 23, 59), midnightNow)).toBe('yesterday');
    expect(dateBucketFor(iso(2026, 8, 16, 0, 5), midnightNow)).toBe('today');
  });
