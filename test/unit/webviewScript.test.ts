...
describe('chatScript', () => {
  ...
  it('外周の枠色は、バックグラウンドターミナルが残っていれば黄になる（Issue #905）', () => {
    ...
    // backgroundRunning列は拡張側のChatStateから判定しており、webviewだけが知る
    // セカンドオピニオンの実行状態を含められないため、ここも条件を合わせた）
    expect(source).toContain('function applyBackgroundRunning(busy)');
    expect(source).toContain("'background-running', !busy && hasBackgroundTerminals");
    // secondOpinionRunningメッセージ自体（ボタンの二重起動防止）は残るが、
    // 枠色の判定用の変数代入は無くなっている
    expect(source).not.toContain('secondOpinionRunning = data.running');
    expect(source).not.toContain('let secondOpinionRunning');
...
describe('workflowScript の全体進捗バー（issue #754）', () => {
  it('区画の集計は拡張機能側の結果をそのまま当てる', () => {
    ...
    // 1通の`feed`メッセージの中の`state`から受け取る（Issue #1272）
    expect(source).toContain('s.progressSegments');
...
describe('workflowScript のタスクごとの model / effort 表示（Issue #1035）', () => {
  ...
  it('承認行・展開行の colSpan は列数の定数に追随し、thead の列数と一致する', () => {
    const source = workflowScript();
    expect(source).toContain('const TASK_TABLE_COLUMNS = 14;');
  ...
  it('狭幅のカード表示でも列の見出しが1つずつずれて揃う', () => {
    const styles = workflowStyles();
    expect(styles).toContain("td:nth-child(6)::before { content: '完了根拠'; }");
    expect(styles).toContain("td:nth-child(10)::before { content: 'model'; }");
    expect(styles).toContain("td:nth-child(11)::before { content: 'コンテキスト'; }");
    expect(styles).toContain("td:nth-child(14)::before { content: '操作'; }");
    expect(styles).toContain('#taskTable tr.task-row td:nth-child(14) { grid-column: 1 / -1; }');
    expect(styles).not.toContain("td:nth-child(13)::before { content: '操作'; }");
  });
