import { describe, expect, it } from 'vitest';
import { ClaudeStreamSession } from '../../src/claude/streamSession';
import type { Logger } from '../../src/log';

const fakeLogger: Logger = {
  info: () => undefined,
  warn: () => undefined,
  error: () => undefined,
  show: () => undefined,
};

interface FakeProc {
  killed: boolean;
  stdin: { write: (line: string) => void; destroyed: boolean; writable: boolean };
}

/**
 * `start()` は実プロセスを起動するため、ここでは `proc` に書き込みを記録するだけの
 * フェイクを直接差し込む（`claudeStreamSessionMcpStatus.test.ts` と同じ方針）。
 *
 * `killed` / `stdin.destroyed` / `stdin.writable` は`write()`が書き込み前に見る生存判定
 * （issue #155、`src/process/stdinSafety.ts`）が通るよう「生きているプロセス」を模す。
 */
function createSessionWithFakeProc(): {
  session: ClaudeStreamSession;
  written: string[];
} {
  const written: string[] = [];
  const session = new ClaudeStreamSession(
    () => 'claude',
    fakeLogger,
    () => undefined,
  );
  const fakeProc: FakeProc = {
    killed: false,
    stdin: { write: (line) => written.push(line), destroyed: false, writable: true },
  };
  (session as unknown as { proc: FakeProc }).proc = fakeProc;
  return { session, written };
}

function controlResponseLine(requestId: string, response: unknown): string {
  return `${JSON.stringify({
    type: 'control_response',
    response: { subtype: 'success', request_id: requestId, response },
  })}\n`;
}

function controlErrorLine(requestId: string, error: string): string {
  return `${JSON.stringify({
    type: 'control_response',
    response: { subtype: 'error', request_id: requestId, error },
  })}\n`;
}

function commandsChangedLine(commands: Array<{ name: string; description?: string }>): string {
  return `${JSON.stringify({ type: 'system', subtype: 'commands_changed', commands })}\n`;
}

describe('ClaudeStreamSession.reloadSkills（issue #202、design.md TP-90）', () => {
  it('reload_skillsのcontrol_requestを書き込む', () => {
    const { session, written } = createSessionWithFakeProc();
    void session.reloadSkills();

    expect(written).toHaveLength(1);
    expect(JSON.parse(written[0]!.trim())).toEqual({
      type: 'control_request',
      request_id: 'req_1',
      request: { subtype: 'reload_skills' },
    });
  });

  it('応答が届くと読み直したskills一覧で解決する（1回目と2回目で一覧が置き換わる）', async () => {
    const { session, written } = createSessionWithFakeProc();

    const firstPromise = session.reloadSkills();
    const firstRequestId = (JSON.parse(written[0]!.trim()) as { request_id: string }).request_id;
    session.receive(
      controlResponseLine(firstRequestId, {
        skills: [{ name: 'add-rule', description: 'ルールを追加する (user)' }],
      }),
    );
    const first = await firstPromise;
    expect(first).toEqual({
      ok: true,
      skills: [
        {
          key: 'add-rule',
          name: 'add-rule',
          description: 'ルールを追加する',
          origin: 'user',
          originDetail: undefined,
          enabled: true,
          toggleable: false,
        },
      ],
      warnings: expect.any(Array),
    });

    // ディスク上へskillが増えた後、もう一度読み直す。一覧が新しい内容へ置き換わることを確かめる
    const secondPromise = session.reloadSkills();
    const secondRequestId = (JSON.parse(written[1]!.trim()) as { request_id: string }).request_id;
    session.receive(
      controlResponseLine(secondRequestId, {
        skills: [
          { name: 'add-rule', description: 'ルールを追加する (user)' },
          { name: 'zzz-temp', description: '増えた一時skill (project)' },
        ],
      }),
    );
    const second = await secondPromise;
    if (second === undefined || !second.ok) {
      throw new Error('reloadSkillsが失敗またはundefinedで解決した');
    }
    expect(second.skills.map((s) => s.name)).toEqual(['add-rule', 'zzz-temp']);
  });

  it('あわせて届くcommands_changed通知で、会話のスラッシュコマンド候補も入れ替わる', () => {
    const commandsSeen: string[][] = [];
    const session = new ClaudeStreamSession(
      () => 'claude',
      fakeLogger,
      () => undefined,
      undefined,
      (commands) => commandsSeen.push(commands.map((c) => c.name)),
    );
    const fakeProc: FakeProc = {
      killed: false,
      stdin: { write: () => undefined, destroyed: false, writable: true },
    };
    (session as unknown as { proc: FakeProc }).proc = fakeProc;

    session.receive(
      commandsChangedLine([
        { name: 'add-rule', description: '' },
        { name: 'zzz-temp', description: '' },
      ]),
    );

    expect(commandsSeen).toEqual([['add-rule', 'zzz-temp']]);
    expect(session.commands.map((c) => c.name)).toEqual(['add-rule', 'zzz-temp']);
  });

  it('応答がエラーなら reason 付きの失敗で解決する', async () => {
    const { session, written } = createSessionWithFakeProc();
    const promise = session.reloadSkills();
    const requestId = (JSON.parse(written[0]!.trim()) as { request_id: string }).request_id;

    session.receive(controlErrorLine(requestId, 'Unsupported control request subtype'));

    await expect(promise).resolves.toEqual({
      ok: false,
      reason: 'Unsupported control request subtype',
    });
  });

  it('プロセスが無ければ何も書き込まず、undefinedで即解決する（会話が閉じている＝対象外）', async () => {
    const session = new ClaudeStreamSession(
      () => 'claude',
      fakeLogger,
      () => undefined,
    );
    await expect(session.reloadSkills()).resolves.toBeUndefined();
  });

  it('dispose()すると応答待ちのreloadSkillsもundefinedで解放される（放置して待たせ続けない）', async () => {
    const { session } = createSessionWithFakeProc();
    // `kill()`はSIGKILLエスカレーション（issue #402）で`once('exit', ...)`も呼ぶため
    // 併せて用意する（実際の`ChildProcessWithoutNullStreams`はEventEmitterなので常に持つ）
    (
      session as unknown as {
        proc: FakeProc & { stdin: { end: () => void }; kill: () => void; once: () => void };
      }
    ).proc.stdin.end = () => undefined;
    (session as unknown as { proc: { kill: () => void } }).proc.kill = () => undefined;
    (session as unknown as { proc: { once: () => void } }).proc.once = () => undefined;

    const promise = session.reloadSkills();
    session.dispose();
    await expect(promise).resolves.toBeUndefined();
  });
});
