 ...
 *
 * すでに `running` / `waitingApproval` のタスクは、依存関係上ここには含まれ得ない
 * （スケジューラが依存未完了のタスクを開始しないため）が、念のため走行中のものは
 * 触らない。すでに `done` / `failed` / `skipped` で確定しているものも上書きしない。
 */
function markFailed(
  run: RunState,
...
export function applyLoopStopReason(
  ...
  if (reason === 'manual' || reason === 'interrupted') {
    ...
    // failedの確定と同じく、まだ開始されていないpendingは実行全体の停止としてskipにする。
    // このタスク自身（現在runningのはず）は変えない。
    const nextTasks = new Map(run.tasks);
    skipRemainingPending(nextTasks);
  ...
  // reason === 'failed'。retriesの範囲内なら、新しいスレッド・worktreeでやり直す前提で
  // pendingへ戻す（送信回数もリセットする）。使い切っていればfailedとして確定する。
