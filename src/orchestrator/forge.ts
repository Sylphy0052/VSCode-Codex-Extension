...
/**
 * 各手順の成否。push・PR/MR作成・ready化は `ok: false` でも後続の手順（マージ）を止めない。
 * レビュー（`reviewPullRequest`）だけは `ok: false` でマージを止める（後述、Issue #1110）。
 */
...
export interface TaskPullRequestFlowResult<TMerge> {
  ...
  /**
   * マージの結果。レビューが `ok: false` を返して止めた場合は `undefined`
   * （マージ自体を試みていない。Issue #1110）。
   */
  mergeOutcome: TMerge | undefined;
 ...
 * `mergeAndPushIntegration` の前に呼ぶ。**`ok: false`（指摘あり・壊れた応答・レビュー自体の
 * 失敗）ならマージとready化を行わない**（design.md §16.18 の手順3.5。指摘のある変更が統合へ
 * 入るのを止める。Issue #1110）。この停止条件はレビューだけのもので、push・PR/MR作成・
 * ready化の失敗は従来どおりワークフローを止めない。
 *
 * 止めた場合 `mergeOutcome` は `undefined` になる。呼び出し側は「マージを試みて失敗した」
 * のではなく「マージへ進まなかった」として、タスクを未解決レビューとして扱う。
 *
...
export async function runTaskPullRequestFlow<TMerge>(
  ...
  // レビューで止める場合は、マージもready化も行わない（Issue #1110）。ready化まで止めるのは、
  // 統合へ入れていない変更のDraftを外すと「マージ済み」に見えてしまうため
  if (review !== undefined && !review.ok) {
    return { pullRequest, review, mergeOutcome: undefined };
  }
