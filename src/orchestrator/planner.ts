...
export function buildSchemaDescription(options: SchemaDescriptionOptions = {}): string {
  ...
  return [
    ...
    '**依存タスクの結果を渡すときは {{<id>.brief}} と {{<id>.handoff}} の組を使うこと。** ' +
      'brief は上流の要点（決めたこと・未解決・変更したファイル・成果物の在り処）だけを短く並べたもので、' +
      'handoff は上流の応答本文を取り寄せるための read_handoff の呼び出し方を示す。' +
      '下流のタスクは、要点で足りるならそのまま進み、全文が要るときだけ read_handoff で取りに行けばよい。',
    '{{<id>.result}} は上流の応答全文をプロンプトへそのまま貼るため、' +
      '参照する依存が増えるほど下流が読む量が積み上がる。全文を前提にしなければ書けない指示に限って使うこと。',
...
export function buildPlannerPrompt(input: BuildPlannerPromptInput): string {
  ...
  if (input.roadmapMaterial !== undefined && input.roadmapMaterial !== '') {
    parts.push(
      ...
      // Issue #1034: 「## 分解の指針」の「原則1〜3タスク」と、この節の「1項目=1タスク」は
      // 項目が4件以上のときに衝突する。どちらが勝つかを書いていないと、材料の項目を
      // まとめたYAMLが返って`detectRoadmapMaterialMismatches`に落ちる
      '- この節の規則は、後述の「## 分解の指針」より優先する。項目が4件以上ある場合も、原則1〜3タスクではなく項目と同じ数のタスクを作ること',
      ...(input.team === true
        ? [
            // ロードマップ経路でのroleは、項目の内容から推定する付加情報でしかない。
            // 「roleごとにタスクを作る」誤りはIssue #842で一度起きているので、
            // 材料の転記制約の側にも書いて二重に効かせる
            '- チームモードでも、この1項目=1タスクの対応を保ったまま各タスクへ主担当のroleを1つ足すだけにする。roleはタスクの追加・削除・分割・統合や、依存関係を変える理由にしてはならない',
            '- 同じroleを複数のタスクへ付けてよい（roleはタスクidではない）',
          ]
        : []),
      '',
    );
