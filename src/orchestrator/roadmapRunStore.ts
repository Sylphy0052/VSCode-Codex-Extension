 ...
 * 通して直列化する（直列化と保存の骨組みは`mementoRunStore.ts`）。`workspaceState`は
 * 暗号化されない平文ストレージのため、応答本文や会話履歴は持たない（`RoadmapRun`が
 * 持つのは状態と識別子、外部由来のIssueタイトルだけ）。
 *
...
export class RoadmapRunStore extends MementoRunStore<RoadmapRun> {
  ...
  hasSessionRef(sessionId: string): boolean {
    ...
    return this.list().some(
      (run) =>
        (Array.isArray(run.orchestratorSessionRefs) &&
