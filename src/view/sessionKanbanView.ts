...
function dropStaleDetails(counts) { const alive=new Set(counts.approvalPending.map(cardKey)); for(const key of [...expanded]) { if(!alive.has(key)) { expanded.delete(key); details.delete(key); detailErrors.delete(key); } } const live=new Set(); for(const spec of specs) for(const card of counts[spec.key]) live.add(cardKey(card)); for(const key of [...turnsExpanded]) { if(!live.has(key)) { turnsExpanded.delete(key); turns.delete(key); turnsErrors.delete(key); turnsInflight.delete(key); } } for(const key of [...btwRuns.keys()]) { if(!live.has(key)) { btwRuns.delete(key); btwInflight.delete(key); btwDrafts.delete(key); btwExpanded.delete(key); } } for(const key of [...cardsExpanded]) { if(!live.has(key)) cardsExpanded.delete(key); } }
