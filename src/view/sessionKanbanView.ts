...
function windowLabel(card) { if(card.isCurrentWindow) return 'このウィンドウ'; if(!windowAliases.has(card.windowId)) { windowAliasCounter += 1; windowAliases.set(card.windowId, windowAliasCounter); } return 'ウィンドウ' + windowAliases.get(card.windowId); }
let toastTimer;
