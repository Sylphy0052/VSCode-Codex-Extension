...
export class PendingStartRegistry<T> {
   ...
   * 通知を取りこぼす。
   *
   * 2件以上あるときは諦める（誤配送を避けるほうが、取りこぼしより重い）。
   *
   * **通知の救済専用**。承認要求の宛先解決には使わない（F10-01）。開始待ちが1件で
   * あることは、未登録のthreadIdがその会話宛である証明にはならず、閉じた会話の
   * 遅延要求などをその1件へ渡すと「別の会話の操作を勝手に許可する」事故になる。
   * 取りこぼして困るのは通知だけで、要求は宛先不明として拒否すればよい。
   */
