...
export class ForgeHubViewManager implements vscode.Disposable {
  ...
  private async receive(message: unknown): Promise<void> {
    ...
    if (message.type === 'startIssue' && this.snapshot !== undefined) {
      ...
      const sessionId = await this.orchestrator.send(
        this.snapshot.provider,
        this.snapshot.cwd,
    ...
    const confirmed = await vscode.window.showWarningMessage(
      `「${draft.title}」のIssue作成をForgeオーケストレータへ依頼します。`,
      { modal: true },
      '作成を依頼する',
    );
    if (confirmed !== '作成を依頼する') return;
    await this.orchestrator.send(
      this.snapshot.provider,
      this.snapshot.cwd,
      `${this.snapshot.host === 'gitlab' ? '$gitlab-issue' : 'GitHub Issueを作成してください。'}\n\nタイトル: ${draft.title}\n\n${buildForgeIssuePrompt(draft)}`,
    );
    this.post({ type: 'issueResult', ok: true });
  }
...
function buildForgeIssuePrompt(draft: ForgeIssueDraft): string {
  return [
    '着手前の現状:',
    draft.currentState,
    '',
    ...
    '',
    'labels:',
    draft.labels?.trim() || '（指定なし）',
    'assignees:',
    draft.assignees?.trim() || '（指定なし）',
    'milestone:',
    draft.milestone?.trim() || '（指定なし）',
  ].join('\n');
}
...
function render(webview: vscode.Webview): string {
  ...
  return `<!DOCTYPE html><html lang="ja"><head><meta charset="UTF-8"><meta http-equiv="Content-Security-Policy" content="${chatCsp(webview.cspSource, nonce, { includeImgData: false })}"><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>${styles}</style></head><body><main><header><div><p class="eyebrow">DEVELOPMENT CONTROL CENTER</p><h1>Forge Hub</h1><p id="subtitle">GitHub/GitLabの状態を確認しています。</p></div><button id="refresh" type="button">更新</button></header><section class="status" id="status" aria-live="polite"></section><section class="hostChoice" id="hostChoice" aria-live="polite"></section><section class="panel orchestrator"><h2>Forgeオーケストレータ</h2><p id="orchestratorStatus">会話を開始すると、ここで進捗と確認要求を確認できます。</p><div id="orchestratorMessages" class="orchestratorMessages" aria-live="polite"></div><div id="orchestratorApprovals" class="orchestratorApprovals"></div><div class="composer"><textarea id="orchestratorInput" placeholder="例:#123を着手して、$gitlab-reviewでこのMRを確認して"></textarea><button id="sendOrchestrator" class="primary" type="button">送信</button></div></section><section class="boardWrap"><h2>Forgeカンバン</h2><div id="board" class="board" aria-label="Forge作業の状態"></div></section><section class="grid"><section class="panel"><h2>Issueを作成</h2><p>内容は同じForgeオーケストレータへ渡し、Issue作成skillで処理します。</p><label>タイトル<input id="title" required></label><label>labels（カンマ区切り）<input id="labels"></label><label>assignee（カンマ区切り）<input id="assignees"></label><label>milestone<input id="milestone"></label><label>着手前の現状（必須）<textarea id="currentState" required></textarea></label><label>非エンジニア向け概要<textarea id="overview"></textarea></label><label>エンジニア向け仕様・実装計画<textarea id="implementation"></textarea></label><label>確認者向け確認点<textarea id="verification"></textarea></label><button id="createIssue" class="primary" type="button">オーケストレータへ依頼</button><p id="result" role="status"></p></section><section class="panel next"><h2>既存Issueから着手</h2><p>選択すると、同じ会話へ実装開始skillの依頼を送ります。</p><button id="listIssues" type="button">Issueを読み込む</button><div id="issues" class="issues" aria-live="polite"></div><p id="startResult" role="status"></p></section></section></main><script nonce="${nonce}">${hostSelectionScript}${script}${reviewRefreshScript}${orchestratorScript}</script></body></html>`;
}
...
const reviewRefreshScript = `(()=>{const vscode=acquireVsCodeApi();const labels={inProgress:'実装を続ける',review:'レビューを進める',ci:'レビュー・マージ準備を進める',cleanup:'cleanupを進める',blocked:'ブロックを調査する'};window.addEventListener('message',event=>{const data=event.data;if(data.type==='reviewResult')document.getElementById('startResult').textContent=data.ok?'レビューを更新しました。':data.message;if(data.type!=='snapshot')return;for(const item of data.workItems||[]){for(const card of document.querySelectorAll('.workCard')){if(card.querySelector('.branch')?.textContent!==item.branch)continue;const actions=card.querySelector('.actions');actions.replaceChildren();const button=document.createElement('button');button.type='button';button.textContent=labels[item.status]||'対応する';button.addEventListener('click',()=>vscode.postMessage({type:'runWorkAction',branch:item.branch}));actions.append(button);if(item.pullRequestUrl){const link=document.createElement('a');link.textContent='PR/MRを開く';link.href=item.pullRequestUrl;link.target='_blank';link.rel='noreferrer';actions.append(link)}if(item.reviewCommentCount!==undefined){const summary=document.createElement('span');summary.className='ciMessage';summary.textContent='レビューコメント: '+item.reviewCommentCount+'件';card.insertBefore(summary,actions)}if(item.reviewMessage){const message=document.createElement('span');message.className='ciMessage';message.textContent='レビュー: '+item.reviewMessage;card.insertBefore(message,actions)}for(const comment of item.reviewComments||[]){const note=document.createElement('span');note.className='ciMessage';note.textContent='@'+comment.author+': '+comment.body;card.insertBefore(note,actions)}}}})})();`;
