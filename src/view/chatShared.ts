    ...
    <div id="composerIconRow">
      ...
      <div id="composerOverflow">
        ...
        <div id="composerOverflowMenu" role="menu" hidden>
          ...
          <button id="loopAdvisorToggle" type="button" class="secondary" role="menuitem" aria-pressed="${options.loopAdvisorEnabled === true}" aria-label="ループAdvisorを${options.loopAdvisorEnabled === true ? '無効にする' : '有効にする'}" title="ゴール駆動ループの各ターンのあとに、別のAI（Codex）へ進め方の妥当性を確認させるか切り替えます。有効にすると相談先はCodex固定になり、無効に戻しても相談先の設定はCodexのまま残ります">${COMPOSER_ICONS.loop}<span class="composerOverflowLabel">ループAdvisorを${options.loopAdvisorEnabled === true ? '無効にする' : '有効にする'}</span></button>
        </div>
      </div>
    </div>
  </div>
