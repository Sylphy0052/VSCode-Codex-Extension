...
export interface ChatShellOptions {
  ...
  /**
   * ループのAdvisor（`agent.chat.loopAdvisor.enabled`、issue #957）が有効か。
...
const COMPOSER_ICONS = {
  ...
  orchestratorMode:
    '<svg viewBox="0 0 16 16" width="14" height="14" aria-hidden="true" focusable="false" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="3.5" r="1.8"/><path d="M8 5.3v2.2M3.5 10V8.5h9V10"/><rect x="2" y="10" width="3" height="3.5" rx=".6"/><rect x="6.5" y="10" width="3" height="3.5" rx=".6"/><rect x="11" y="10" width="3" height="3.5" rx=".6"/></svg>',
  progress:
...
function composerButtonSpec(id: ComposerButtonId, ctx: ComposerButtonContext): ComposerButtonSpec {
  switch (id) {
    ...
    case 'orchestratorMode':
      return {
        ariaLabel: 'オーケストレータモードを開始',
        title: 'Orchestratorが計画したタスクを工程ごとのセッションで並列に進めます',
        hidden: false,
        pressed: false,
        icon: COMPOSER_ICONS.orchestratorMode,
      };
    case 'openProgress':
      return {
        ariaLabel: '進捗を表示',
    ...
    case 'secondOpinion':
      return {
        ariaLabel: 'セカンドオピニオン',
        title:
          'この会話そのものは渡さずに、依頼文と選んだ資料だけを独立したCodexセッションへ渡して意見を求めます（レビューのほか、案の比較や進めてよいかの判断も頼めます。既定では別セッションが作った背景要約を添えます。結果はこの会話へ表示され、作業中のAIへ自動では送られません）',
        hidden: false,
        pressed: false,
