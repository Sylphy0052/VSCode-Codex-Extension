/**
 * 別セッションへ送る直前の credential redaction（Issue #947 受入基準12）。
 *
 * 送る材料には差分だけでなく、ログや設定ファイル・fixtureの中身が入りうる。そこに含まれる
 * 認証情報を、そのまま外へ出さないためにここでかける。
 *
 ...
 *
 * 1回の送信で複数のテキストを別々に伏せることがある。会話へ残す件数を別々に出すと、
 * 利用者はその数を足して読むことになる。送ったのは1本のテキストなので、
