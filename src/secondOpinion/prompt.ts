 ...
 */
export function buildAskGptSecondOpinionPrompt(requestText: string): string {
  const lines = [
    'あなたは、別のAIエージェントが進めている作業について、独立した立場から意見を求められています。',
    ...
    '以下の質問は、そのエージェント自身が、あなたのために組み立てたものです。単体で読めるように書かれています。',
  ...
  // 質問文はMarkdown文書そのもので、内部にコードブロックを持つ。囲まずに置くと、質問文の
  // 見出しと固定指示の地の文が同じ高さに並び、どこからが資料なのかが読み取れなくなる
