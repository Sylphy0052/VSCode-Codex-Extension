...
export interface CreateReviewBundleRequest {
  ...
  /** 押下時に解決したコミット。ここから読む内容は不変である。 */
  baseCommit: string;
  /** 押下時点の差分の全量（上限で切る前のもの）。 */
  fullDiff: string;
...
export async function createReviewBundle(
  ...
  try {
    await fs.writeFile(path.join(dir, REVIEW_BUNDLE_DIFF_FILE), request.fullDiff, 'utf8');
    const baseDir = path.join(dir, REVIEW_BUNDLE_BASE_DIR);
    await fs.mkdir(baseDir, { recursive: true });
    let used = 0;
    let written = 0;
    for (const relative of request.changedPaths) {
      ...
      const shown = await request.git.run(
        ['show', `${request.baseCommit}:${relative}`],
        request.cwd,
      );
      if (shown.code !== 0) {
        continue;
    ...
    request.log?.info(
      `${LOG_PREFIX} bundle created baseFiles=${written}/${request.changedPaths.length} ` +
        `diffChars=${request.fullDiff.length}`,
    );
    return bundle;
  } catch (e) {
    // 途中まで書いたディレクトリを残さない
