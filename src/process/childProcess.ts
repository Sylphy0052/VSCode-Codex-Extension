...
/**
 * app-serverとのJSON-RPCに限って使う、1行分のバッファ上限（issue #795）。
 *
 * `thread/fork` と `thread/resume` の応答はスレッドの会話アイテムを全件含み、それが
 * 改行区切りJSON-RPCの1行として届く（design.md §14.26。fork応答は`thread/resume`と
 * 同じ形）。つまり応答の大きさは会話量に比例して伸びる性質のもので、
 * {@link MAX_LINE_BUFFER_BYTES}（1メッセージに画像1枚が入っても壊れない大きさとして
 * 決めた10MB）では足りない。実測では`~/.codex/sessions`のロールアウトは1122件中43件が
 * 10MBを超えており（2026-08-25、最大160MB）、会話が長いセッションは分岐も再開も
 * できなくなっていた。現在確認できるロールアウトは最大約531MiBであり、128MBでは
 * 履歴の復元も同じ理由で失敗する。
 *
 * 上限そのものは撤廃しない。撤廃すると壊れた出力を延々と連結してメモリを食い潰す経路が
 * 復活するため（issue #402、1点目）。会話量に比例して伸びる応答を通せて、かつJavaScriptの
 * 文字列として保持しても現実的な大きさとして384MBを選んだ。
 *
 * **V8の文字列長上限（`buffer.constants.MAX_STRING_LENGTH` = 536870888）より下でなければ
 * ならない**（issue #1153）。`FrameBuffer`はチャンクを文字列へ連結してから上限を判定する
 * 作りで、上限が文字列長上限以上だと、判定へ到達する前に連結が`RangeError: Invalid string
 * length`で落ちる。当初の512MB（536870912）は文字列長上限より24バイト大きく、改行を含まない
 * 出力が溜まり続けても`overflow`が一度も立たなかった（実測、2026-09-13。535822336バイトの
 * 投入でRangeError）。つまりissue #402の保護がこの経路だけ効いていなかった。
 *
 * 判定は連結の**後**に行うため、ピーク時の文字列長は「この上限＋直前に受け取ったチャンク1個分」
 * になる。384MBなら文字列長上限まで128MBの余裕が残り、チャンク1個分（stdioのパイプでは通常
 * 64KB）を大きく上回る。なお、UTF-8のバイト数はUTF-16のcode unit数以上なので、「バイト数が
 * 上限以下」なら「文字数も上限以下」が保証される。バイト側だけを見れば足りる。
 *
 * 実測できている最大の応答は227.7MiB（531MiBのロールアウトの`thread/resume`、2026-09-13）で、
 * 384MBはおよそ900MiBのロールアウトまで通る。
 */
export const MAX_APP_SERVER_LINE_BYTES = 384 * 1024 * 1024;
