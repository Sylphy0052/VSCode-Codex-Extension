...
- **外向きの副作用は、必ずセキュリティゲートの後ろへ置く。** 拒否したはずの作業の痕跡が
  外部ホストに残る形は、テストが緑でも人が気づけない。WF-E の W6（Issue #596）で、
  タスクごとのIssue起票が `bypassPermissions` の最終防御より前に走っており、
  **「危険判定が働かない設定なので開始できません」と拒否したタスクについて、外部ホストに
  Issueが1件残っていた。** 直すときは順序を入れ替えるだけでなく、**順序を固定する回帰テストを
  置く**こと——順序は書き換えても型が通るので、テストが無ければ次の変更で簡単に戻る
