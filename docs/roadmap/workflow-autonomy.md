...
- [x] W3 生成したワークフローの分解が妥当かをレビューする段を足す
  ...
  - 変更: 生成したYAMLを別の読み取り専用セッションでレビューさせる段を足す。観点は4つ
    ...
    ゴールに対して過不足がないか）。指摘があれば、別の読み取り専用セッションへ指摘を渡して
    YAMLを修正させ、構文検証後に再レビューする。**YAMLファイルへ反映するのは拡張側だけ**で、
    利用者がエディタで編集していた場合は上書きせず止める。無限ループを避けるため修正は最大3回とし、
    なお残る指摘は警告として出す
  - 受入基準: 指針違反があれば自動修正後に再レビューされる／指摘が無くなれば修正済みYAMLが保存される／
