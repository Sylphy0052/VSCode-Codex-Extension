...
- **第7回**: T26の後半（ラベル表）は**着地した**（PR #655、`0992379e`。Issue #649 は CLOSED）。
  ...
  **この2件目は、検算スクリプトのPASSでは捕まっていない。**PASSを見て「奪取なし」と
  ...
  置き換わっている
- **第8回**（完了）: 全体レビュー。**18項目（Issue 16件＋T26＋全体レビュー）が全て CLOSED になった。**
  現在 OPEN なのは実機確認 #189〜#194 の6件のみで、これは18項目に含まれない別枠。

  **JSDocが宣言から切り離される欠陥の全域測定（#670 → PR #672、#671 → PR #673）。**
  第4回（PR #653）・第7回（T26、PR #655）で個別に見つかっていた形を、初めて `src` 全域
  （179ファイル）で測った。宙に浮いたJSDoc（`/**...*/` の直後の最初の非空行がまた `/**` で
  始まる形）は着手前 **63件**。うち **17件が欠陥**（残り46件は import 直後のモジュール説明・
  節の区切り注記など、宣言に付ける意図がないもの）。

  - **挿入型 11件**（#670）: 既存の宣言とそのJSDocの間へ別の宣言を差し込む。第4回・第7回と
    同じ形
  - **重ね書き型 5件 + 移動元の置き去り 1件**（#671）: 既存JSDocへ追記せず、その下に2つ目の
    `/** ... */` を書いて元の説明を押し出す。**この形は今回はじめて見つかった**

  修正後は 63 → **52**（#672後）→ **46**（#673後）。**この件数と内訳（11/5+1）は
  `origin/main` 上の3時点で引き直して一致を確認したが、46件それぞれが本当に欠陥でないかの
  目視確認はしていない。**「import直後のモジュール説明・節の区切り注記に見える」は出力の
  見た目からの推測であり、全件を1件ずつ読んで裏付けたものではない。

  **棚に残す価値があるのは検査の限界。**第7回の記述にある「`ops-rules.md`『抽出を書いたら、
  その抽出が陽性を出すことも確かめる』の直接の由来はここである」検算スクリプト（ベースと
  HEADで「`*/` の直後にある宣言名」の集合差を取る形）は、**重ね書き型を原理的に拾えない。**
  押し出された説明が宣言から切り離されても、宣言の直上には新しく書いた側のJSDocが残るため、
  old にも new にも同じ宣言名が出て、集合差に現れない。**陽性対照を取ってもPASSになる。**
  全域で両方の経路を拾える条件は「JSDocブロックの次の非空行が、また `/**` で始まる」で、
  これが `ops-rules.md`「検査が拾える形は1つだけ。同じ欠陥でも経路が違えば別の抽出が要る」
  （PR #674）の由来。

  実害が最大だったのは `src/orchestrator/runnerOrchestrator.ts` の
  `planChangeFinishedReason`——Issue #339 のレビュー指摘に対するA/B/C選定理由を含む49行の
  設計判断の記録が、どの宣言にも付いていない状態だった。ほか `planner.ts` の
  #58セキュリティ監査criticalの経緯22行、Issue #389 のタイムアウト設計25行が押し出されていた。

  **ラベル表の網羅測定（#668 → PR #669）。**差分の種類ラベル（`chatScript.ts` の
  `kindLabel`）に網羅の突き合わせが無いことが分かり着地した。第7回に書かれているラベル表の
  件数の腐りと同じ系列。**Issue #668 本文に2件の誤りがあった**——(a)「`LOOP_STOP_LABEL` と
  同じ string literal union なので既存の正規表現が使い回せる」は実測で0件（`FileDiffKind` は
  1行書きのため）、(b)「実測 9+8+11=28」は2つの辞書が両方の群に入る二重計上で、正しくは
  9+6+11=26。どちらも担当セッションが指摘し、Issueへコメントで訂正した（本文は起票時点の
  記録として編集していない）。

  **第8回で起きた運用の誤り（全体オーケストレーターのもの）。**`ops-rules.md` への転記を
  WF-Gの担当以外のセッションへ振った。上の「#636」の項目が「WF-Gは渡すところまで、書くのは
  全体オーケストレーター」と定めているのに、その規約を読まずに振っている。PR #674 は
  着地・検算済みだが、追加した節の「全体オーケストレーターが転記した」という文面と、実際に
  転記した主体が食い違っている。**次に `ops-rules.md` を触るときは、全体オーケストレーターが
  自分で書く。**担当が文面を起こすところまでは規約どおりで、転記の段で渡さない。
