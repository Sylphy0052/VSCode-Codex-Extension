...
- **第5回**（完了）: #599（PR #657、`60078b90`）+ #524（PR #658、`cd798eeb`）。
  ...
  **同じ本文の箇条書きは7つを挙げていた**（名前付き5つ＋PR #518 で足した2箇所）。
  ...
  分けて書く」
- **回の外で消化中**: #491。**回の構成のどこにも置かれていなかった1件である。**
  ...
  **#491 は他の項目と性質が違う。**単に1件を消化するのではなく、**#432-2 で既に着地している
  受入基準3件を明示的に上書きする**——終了したrunを `retry_task` で再開したとき
  `finishedNotified` をリセットする方向へ変えるため、「再開してもここは変わらない」を
  守っていた既存のテストが、そのまま残せない。**既に下した判断を置き換える回は、
  この棚に他に無い。**上書きした事実と、#432-2 が何を守るための判断だったかは
  design.md 側に残す。
