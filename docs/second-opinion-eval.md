...
#### 途中で止まったら同じディレクトリで再開する（Issue #1310）

144往復は実測で10時間以上かかる。1つのプロセスが最後まで生きている前提は置けない（2026-09-19の本測定は33往復目で、実行していたセッションごと終了した）。

`--out` に `manifest.json` があるときは、`run.ts` は新しいrunを始めず、**そのrunの続きとして実行する**。同じコマンドをもう一度打てばよい。

- `runId` を引き継ぐ。引き継がないと、既にある結果が採点シート生成時に「別run」として捨てられる
- 成功している往復だけを飛ばす。`error` を持つ結果と本文が空の結果は**作り直す**。前回の失敗をそのまま成果に混ぜると、失敗した条件だけ件数が減ったまま採点へ進むことになる
- ある案件の全ての（条件 × 試行）が成功済みなら、材料の準備ごと飛ばす
- 再開したことは `manifest.json` の `resumes`（`harnessCommit` / `startedAt` / `skipped`）に残る。再開時のハーネスのcommitは初回と違いうるので、`harnessCommit` 1つだけでは実際に測ったコードを言えない

`--cases` / `--eligibility` の内容ハッシュ、`--model` / `--effort` / `--conditions` / `--attempts` / 案件数のどれかが既存の `manifest.json` と食い違うときは、**1件も実行せずに終了コード1で止まる**。警告にして続けると、条件や正解ラベルの違う結果が同じ `runId` で1つのディレクトリへ混ざり、採点シートは `runId` しか見ないので件数にも現れない。別の測定を始めるときは別のディレクトリを渡す。
