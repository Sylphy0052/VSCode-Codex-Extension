...
`cwd` を空の一時ディレクトリ（`os.tmpdir()` の下に `mkdtemp`）へ向け、成功・失敗・打ち切りのいずれでも `finally` で消す。プロンプトにも「ファイルを読む必要はありません」と書く。作れなかった場合は警告を1行残して従来どおり実workspaceで走らせる（隔離できないことを理由に要約を落とさない）。
