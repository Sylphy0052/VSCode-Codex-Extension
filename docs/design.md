...
**更新は材料の差し替えではなく、同じbundleへの追加にした。** Advisorのセッションの作業ディレクトリは `turn/start` を出した時点で固定されており、後から差し替える手段が無い。セッションを開き直せば差し替えられるが、それは相談の継続そのものを捨てることになる。そこで、同じbundleの中に世代を積む。

- 1世代目 — `changes.diff` / `base/<パス>`（Eのまま）
- 2世代目以降 — `updates/<世代>/changes.diff` / `updates/<世代>/base/<パス>`

古い世代は消さない。更新の前後で何が変わったのかをAdvisor自身が読めるようにするためで、まとめて消えるのはbundleを `dispose()` するときである（後始末の経路はEから変えていない。相談の終了・親の破棄・拡張の終了・異常終了後の回収のいずれでも、bundleごと消える）。

**更新は利用者が押したときにだけ起きる。** 自動更新にすると、押していないのにAdvisorの前提が入れ替わり、同じ問いに同じ答えが返らなくなる。何を根拠に答えたのかを利用者が辿れる状態を保つ方を採った。

更新の手順は「新しい世代を書き出す → Advisorへ1ターン使って伝える」の順である。逆順にすると、存在しない材料を正本だと伝えることになる。伝える本文（`buildMaterialUpdatePrompt`）は、新しい材料の場所を名指しし、「以後はこちらを正本とする」「これまでの議論と食い違う部分があれば指摘せよ」の2点を伝える。ここで新しい問いは立てない——通知と一緒にレビューを始めさせると、利用者が頼んでいない観点の回答を待たされる。

**世代を進めるのは、通知のターンが成立したときだけである。** 書き出せただけで進めると、Advisorが古い材料のまま話しているのに「最新の世代」と数えることになり、後述の古さの警告が出なくなる。書き出しだけ済んだ世代のディレクトリは、次の更新で同じ番号のまま書き直す（`appendReviewBundleRevision` が書く前に消す）。

**書き戻しのときに古さを示す。** 下書き（`HandoffDraft`）は、作られた時点の材料の世代を持つ。承認の確認ダイアログで、その世代が現在の世代より古ければ、どの世代に基づく下書きなのかを文面に出す。**送信は止めない**——古いと分かったうえで送りたい場合があり、そこを塞ぐと下書きを作り直すためだけに1ターン使わせることになる。止めるのは「知らないまま送ること」だけである。
