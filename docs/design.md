...
§14.104 で送る量を減らしたが、**webview側は届いた内容で毎回全DOMを作り直していた**。`renderTimeline` は `clear(timeline)` してから全ターンを組み立て直し、`renderFiles` も同様だった。応答中は間引き後でも毎秒20回ここへ来る。
...
§14.104 で拡張機能側は既に「変わったターンだけ」を送っている。webview側は届いた `payload.turns.turns` の `index` を集めて `render` へ渡し、**その集合に入っているターンだけを作り直す**。残りは `className` と `open` だけを当てて据え置く（最新の印と開閉は中身が同じでも移りうるため）。

全量（`mode: 'full'`）が届いたときと、中身が空になったときは土台（`turnNodes`）ごと捨てて作り直す。

ファイル一覧は差し分が無いので、ディレクトリとファイル名を連ねた文字列を鍵にして前回と突き合わせ、同じなら何もしない。ファイルは1ターンに数件しか増えないため、この鍵の組み立てのほうが全行の作り直しより安い。

「閉じているNターンを開く」ボタンも、Nが変わらなければ作り直さない。押せる要素を作り直すとフォーカスが飛ぶ。

#### 読み上げは状態の1語だけに絞る

`#statusBadge` に `aria-live="polite"` と `aria-atomic="true"` を付け、**応答中↔待機中が移ったときだけ**中身を書き換える（`badgeBusy`）。

KPI 領域へ広く `aria-live` を付ける案は採らない。応答中は毎秒20回状態が届くので、数字を読み上げ対象に含めると更新のたびに読み上げ続ける。**読み上げる内容と、DOMを触る頻度は必ずセットで決める**（片方だけ足すと、live region が正しくてもDOM更新が多すぎて使い物にならない）。

#### 残っている範囲

**最新ターンは毎回作り直す。** 中身が実際に変わっているので据え置けない。応答中に最新ターンの文字を選んでいる間は、選択は依然として消える。古いターンの閲覧・コピーは保たれる。

#### 確かめ方

- `test/unit/progressDom.test.ts`: 据え置き（ノードの同一性）、選択の保持、フォーカスの保持（見出しとボタン）、開閉の保持、全量での作り直し、live region の更新頻度、積み直し失敗時の送り直し
