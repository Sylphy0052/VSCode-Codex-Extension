...
- 承認待ちの列と件数バッジはマゼンタにする。役割（対応を待っているもの）は会話画面の承認カードと同じ

#### 動くもの

承認待ちが1件以上あるときだけ、画面の上端に走査線を1本流す。会話画面が応答中に流すのと同じ役割で、この画面で無限に回るアニメーションもこれだけである。

判定は絞り込み後ではなく盤面全体の件数で行う。絞り込みで隠れただけの承認待ちがあるのに合図が消えると対応漏れを誘うためで、件数バッジの強調（issue #1250）と同じ考え方である。切り替えは描画のたびに`body`の`has-approval`クラスを付け外しするだけで、0件なら要素ごと消える。
