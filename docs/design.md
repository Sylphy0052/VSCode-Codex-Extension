...
再入そのものは止めていない。連打すると**同じ `turnId` で `turn/interrupt` が2回送られる**。表示の上ではidが一致するので注記は1行のままだが、**2回目の要求にapp-serverがどう応えるかは未確認**（issue #261）。エラーを返すなら、要求を2回送らない形（進行中のPromiseを使い回すなど）へ寄せる方が素直になる。
...
4001項目では1回47.78msで、上の50msの間引き幅とほぼ同じになる。送信が終わった直後に次の送信が始まる形になり、直したはずの「イベントループが埋まって `turn/interrupt` の応答を読めない」筋へ戻る。**対処は会話を捨てることではなく、変わった項目だけを送ること**（issue #262）。それなら1回のコストが項目数に依存しなくなり、会話も残せる。
