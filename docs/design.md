...
| `isolation`                               | -    | defaults                | `worktree` / `shared`（§16.6）                       |
| `cwd`                                     | -    | -                       | 明示するとworktreeを作らずそのディレクトリで走らせる |
| `model` `effort` `approvalMode` `sandbox` | -    | defaults→拡張機能の設定 | そのタスクのセッションにだけ効く                     |
| `autoApprove`                             | -    | defaults                | `false` にすると承認を全て人へ回す（§16.7）          |
| `escalate`                                | -    | `[]`                    | 自動承認しないコマンドのパターン追加                 |
| `allow`                                   | -    | `[]`                    | 既定の停止条件から外すパターン                       |
| `retries`                                 | -    | `0`                     | 失敗時の再試行回数                                   |
...
読み込み時に次を検査し、1件でも該当すれば実行を始めない。

- `id` の重複、`dependsOn` の未定義参照
- 依存の循環（トポロジカルソートで検出し、循環に含まれるidを全て示す）
- `done` の欠落、`maxParallel` が1未満
...
タスクが `failed` になった時点で、そのタスクに依存する後続を `skipped` にし、実行全体を止める。独立した枝も止める（合流タスクの前提が崩れた状態で走らせない）。
...
既定は無人実行（`autoApprove: true`）で、承認要求は拡張機能が自動で許可する。ただし**危険と判定した要求だけは自動で通さず、そのタスクを止めて人へ回す**。

- 承認要求は今まで通り受け取る（Codexは `approvalPolicy` を `never` にせず `on-request` のままにする。要求が来なければ判定もできないため）
- 受け取った要求を `escalation.ts` の純粋関数へ渡し、`auto` か `ask` かを決める
- `auto` なら即座に許可を返す。`ask` ならタスクを `waitingApproval` にして、通知とワークフローViewで知らせる。人が決めるまでそのタスクは進まない（他のタスクは走り続ける）

既定で `ask` に倒す対象は次のとおり。

- 削除・巻き戻し: 再帰的な強制削除、追跡外ファイルの一括削除、作業ツリーの強制巻き戻し、ブランチやタグの削除、テーブルの削除や全消去
- 外部へ出る操作: リモートへのpush（特に強制push）、デプロイ、パッケージの公開、外部エンドポイントへの送信
- 作業ディレクトリの外への書き込み（worktree外・ワークスペース外の絶対パス）
- 権限そのものの変更（`permissions` 種別の要求）
- 判定に失敗した・種別が未知の要求

`escalate` で追加、`allow` で除外できる。`allow` はタスク単位に閉じ、ワークフロー全体の既定は緩めない。

サンドボックスは既定で有効のまま（`sandbox: workspace-write`）にする。自動承認とサンドボックス無効化を同時に指定した組み合わせは、実行前に1度だけ確認を挟む（Claude側の `isUnsafeClaudeCombination` と同じ考え方）。
...
- 警告欄: git外フォールバック、サンドボックス無効の指定、回数切れなど
...
3. 受け取ったYAMLを§16.2の検証にかける。通らなければ、検証エラーを添えてもう1度だけ投げ直す
4. `agent.workflows.dir` へ保存し、エディタで開く。ワークフローViewを同時に開き、依存関係の図を見ながら人が直す
5. 人が直したら「実行」で走り出す

生成に使うプロバイダは `defaults.provider` と同じ既定（設定で変更可）。分解セッションはタスクを実行しない（読み取りと提案のみ）。
...
既存のチャット画面に対する変更は次の3点。いずれも既存の呼び出しは既定値でそのまま動く。

1. `ChatViewManager.openNew()` がワークスペース直下に固定されているため、cwdとタスク単位の設定を受け取れるようにする
2. パネルを背面で作れるようにする（`createWebviewPanel` に `preserveFocus` を渡す）
3. **セッションの寿命をパネルから切り離す。** 現状は `panel.onDidDispose` で `session.dispose()` まで行っているため、タブを閉じるとセッションごと終わる。タスクとして走っているセッションは、タブを閉じても破棄せずオーケストレータが持ち続け、`reveal()` でパネルを作り直して `ChatState` から会話を描き直す。承認の保留も同じく持ち越す（閉じた時点で拒否しない）

3の切り離しは、閉じ忘れたセッションが残り続ける危険と裏表なので、範囲をタスク実行中のセッションだけに限る。人が手で開いた画面はこれまで通りタブを閉じたら終わる。ワークフローが終わった（`done` / `failed`）タスクのセッションは、その時点で解放する。

### 16.11 永続化と復元

- 実行中の状態（runId・タスクごとの状態・sessionId・worktreeのパス）は `workspaceState` に置く
- ウィンドウのリロードで走行中だったタスクは、いったん `failed`（理由: 中断）として扱う。チャットタブ自体は既存の復元（§5.5・§9.5）で戻るので、人がViewから再実行できる
- 走り終えたrunの記録も残し、Viewから開き直せるようにする（最新10件）
...
1. `workflow.ts`: YAMLの読み込みと検証、循環検出、テンプレート展開
2. `scheduler.ts`: 依存と `maxParallel` から次に開始する集合を出す
3. `runState.ts`: 状態遷移（失敗時の後続 `skipped` 化を含む）
4. `escalation.ts`: 危険判定
5. `worktree.ts`: 作成・撤去・git判定（コマンド実行はポート越しに差し替え可能にする）
...
- `T1 → (T2 || T3) → T4` のワークフローが定義から最後まで通り、T2とT3が同時に走ることをViewで確認できる
- 依存グラフが3段（T1 / T2・T3 / T4）で描かれ、各ノードの状態が進行に合わせて `pending` → `running` → `done` と変わる
- 実行中のノードから会話のタブへ移動でき、途中経過が読める。タブを閉じてもタスクは走り続け、ノードから開き直すとそれまでの会話が全て復元される
- ノードの `中断` で進行中のターンだけ止まり、次の指示から続く。`タスク停止` でそのタスクだけが止まる
- T2とT3が別のworktreeで動き、互いのファイルを踏まない
- gitリポジトリでないフォルダで実行したとき、警告を出したうえで `shared` で走る
- 危険と判定したコマンドでタスクが止まり、Viewからの許可で再開する
- 依存の循環を含む定義が、実行前にエラーとして弾かれる
- ゴール文からYAMLが生成され、検証を通り、エディタとViewで確認できる
