...
2. まだ超えるなら、自動生成とみなすパス（ロックファイル・`dist/` 等・`*.min.js`・`*.generated.*`）を**大きい順に**落とす。収まった時点で止める——予算に収まるなら自動生成でも渡す（ロックファイルの差分から依存の追加に気付くことはある）
3. まだ超えるなら、残ったファイルへ予算を配分してhunkの境界で切る

配分は小さいファイルから順に「残り予算 ÷ 残りファイル数」を割り当て、使い切らなかった分を次のファイルへ回す。先頭から詰める方式だと出力順で後ろのファイルが必ず落ちるが、パスの並びは変更の重要度と関係が無い。

**ファイルのheaderは予算を超えても必ず入れる。** 予算がhunk 1件分も無い場合、そのファイルは「hunkを全部省略した」形で残る。ファイルの存在自体は消さない——変更の中心が巨大なファイルであることは普通にあり、そこで消えると最も見てほしいものが伝わらない。「1ファイルで予算を超えたらそのファイルを丸ごと省略」は採らない。

落としたものは2つの経路で伝える。差分の本文には `# 省略: このファイルの残り N/M hunk は上限のため渡していません` を挿入し（差分としては不正な行だが、これを読むのは `git apply` ではなくモデルである）、プロンプトには「上の差分に含めなかったもの」の一覧をパス・サイズ・理由付きで出す。一覧が長くなりすぎないよう `MAX_DIFF_OMISSION_ENTRIES`（50件）で打ち切り、残りは件数だけを伝える——省略の一覧そのもので予算を食い潰しては本末転倒である。
