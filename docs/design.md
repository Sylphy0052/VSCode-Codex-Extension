...
##### カンバンバッジから該当タスクを絞り込む（issue #752）

バッジ（`ToDo: 3` / `InProgress: 2` / `Done: 5` / `要対応: 1`）は集計を出すだけで、その件数がどのタスクなのかは自分でグラフから探すことになっていた。バッジを `<button>` にし、押すとそのバケットのタスクだけを強調する（もう一度押すと解除）。

- **該当しないノードは消さずに淡くする**（`.wf-node.dimmed { opacity: 0.25 }`）。依存グラフが主役の画面でノードが消えると、依存関係が読めなくなる
...
- **絞り込み中のバケットが0件になったら解除する**。該当が1つも無いまま「絞り込み中」の見た目だけが残ると、なぜ全部が淡いのかが分からない
- 絞り込みの状態（`kanbanFilter`）はスクリプト側の変数に持つ。状態が届くたびに画面は組み直されるが、この値は残るので絞り込みは維持される
