...
判定を1か所に揃え、`goalLoop.ts`が持つ`isSettledCommandItem`（`collectCommandEvidence`が拾う条件と同じもの）を通ったものだけを記録するようにした。**「拾う条件」と「拾ったと記録する条件」を別々に書いてはいけない。** 長いテストスイートほど当たりやすく、「`status: pass`の機械的な証拠でだけ達成と判定する」という§14.81の前提を静かに壊していた。

#### `pause()`は次の指示の送信だけを止める

`pause()`（§16.21のwaitingReply）は、実装上「ターンの後始末そのものをしない」になっていた。ターンの失敗だけを見て早期に戻るため、一時停止中に完了したターンについて撤退の申告・完了・停滞・時間切れ・回数切れのいずれも判定されない。`resume()`は判定を挟まずに送るため、上限を超えた送信・撤退の申告の握り潰し・走行中のターンへの二重送信が起きた。§14.79で足したBound（外側から縛る側）が、この経路では丸ごと効いていなかったことになる。
...
ゴール駆動に限り、Evaluatorの**終局の判定（`achieved` / `escalate`）だけ**を停滞より先に見るようにした。停滞判定を無効にしたのではない。`continue`・続きを許す`indeterminate`のときは、従来どおり停滞 → 時間 → 回数の順に縛る。
