...
- `markApproved()` は `handoffDrafted` からしか通らない。相談中や閉じた後から承認へ飛べると、「何を承認したのか」が定まらない
...
`parseHandoffDraft`（`src/secondOpinion/handoff.ts`）は厳格に読む。コードブロックがちょうど1つあり、オブジェクトで、2つのキーが空でない文字列で、長すぎないこと。**読めなければ下書きとして扱わない**——形を推測で補って通すと、要約の側の文が指示文へ入り込んだまま承認へ進む経路ができる。読めたときにだけ `markHandoffDrafted()` で状態を進めるので、「承認できるのは読めた下書きだけ」が不変条件になる。
