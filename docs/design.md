...
ただし承認の決定はこの前提に乗せない。Phase 2（issue #1259）では、承認対象の中身を取り寄せてカード上に展開し、それを読んだ上でしか押せない形にする。

#### この段階で入れないもの

承認・拒否を送る経路はPhase 1では作らない。`globalStorage`配下にJSONを1つ置くだけで別ウィンドウにコマンド実行を承認させられる経路になるため、承認対象の中身（コマンド全文・変更対象パス）を取り寄せてカード上に展開し、それを読んだ上でしか押せない形と合わせてPhase 2（issue #1259）で入れる。
