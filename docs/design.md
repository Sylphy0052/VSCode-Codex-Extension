...
この契約は継続ターンを含む全ての指示へ前置される（`composeTaskExecutionContract`。`src/orchestrator/runner.ts`）。拡張機能が送る本文には毎ターン1個しか載らないが、**CLI側の会話履歴には契約入りのメッセージがターンごとに積み上がる**ことをIssue #1320で実測した。同一スレッドへ3ターン送ったときのセッション記録ファイル内の契約数は、Codexが2・4・6（1ターンにつき会話履歴そのものの`response_item/message`とUI向けの`event_msg/item_completed`の2レコードへ同じ本文が書かれるため、1ターンあたり2件増える）、Claude Codeが1・2・3。どちらも増分は一定で、ターン数に比例して契約のコピーがコンテキストを占有する。計測は`npm run measure:contract-history`（`scripts/measure-contract-history.mjs`）で追試できる。認証と利用枠を消費するためCIには載せていない。
