...
PR/MR作成の順序（W-16・W-17。Issue #172）を統合テストへ落とすときも、この設定は`none`のままにする。実ホスト（`gh` / `glab`）を呼ぶ経路はフェイクのポートで差し替え、実際のpushは通さない。
