...
> セカンドオピニオンは、進行中の作業について、作業を担当しているAIとは独立した別セッションへ判断・助言・レビューを求める機能である。別セッションには、会話から独立に生成した背景要約、利用者の今回の質問、および必要に応じて成果物や直近の回答を渡す。回答は元セッションへ表示するが自動送信せず、採否は利用者が判断する。
...
最終回答は全文を親セッションの会話へ1項目として差し込む（要約しない。別モデルの解釈を挟むとセカンドオピニオンとしての値打ちが落ちる）。**ユーザー発言として自動送信はしない。** 意見が誤っていた場合に、それがそのまま次の作業指示として扱われてしまうため。採否を人が決めるこのgateは、目的を「レビュー」から「相談」へ広げても変えない（プロンプトの固定指示にも「回答は利用者へ表示されるだけで、元の作業へ自動では反映されない」と書いてあり、Advisorに作業指示ではなく判断材料を書かせる意味もある）。
