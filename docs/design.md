...
拡張機能から見ると、この要求は120秒の要求タイムアウト（`REQUEST_TIMEOUT_MS`）まで宙に浮いてから失敗する。そのため `ChatSession.interrupt` は**応答を待っている間の呼び出しでは要求そのものを送らない**（`interrupting`）。`Esc` を連打しても送る要求は1回で、2回目以降は何もせずに返る。
