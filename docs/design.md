...
§14.80は会話を一切渡さない設計だった。独立性は最大化されるが、レビュー側は「何をしようとしていたのか」「どこまで確認済みか」を知らないまま差分だけを読むことになり、既に確認済みの点を指摘する・依頼の背景を取り違えるといった外し方をする。会話の要約を添えられるようにした（既定で有効）。
...
セカンドオピニオン本体のプロンプトでは、要約を差分とは別の区画に置き、固定指示へ「要約は会話を見ていない別のセッションが作った圧縮であり、抜けや誤りがありえる。要約と差分が食い違う場合は差分を優先し、食い違い自体を指摘せよ」を足す。レビュー対象の正本はあくまで押下時に固定した差分（§14.80）で、要約はその読み方を助ける文脈でしかない。

会話へ残す注記も書き分ける。要約を添えた場合は「この会話そのものは渡していません（別セッションが作った要約のみを添えた独立評価です）」とし、「会話を渡していない」とだけ出して事実と食い違う状態にしない。

#### 失敗しても本体は止めない

要約は付随情報のため、失敗・空応答・タイムアウトのいずれでも `ok: false` を返すだけで、セカンドオピニオン本体は要約なしで続行する。要約を切る（`agent.secondOpinion.summary.enabled: false`）と、要約セッションを開かず、プロンプトはIssue #894時点と一字一句同じになる。
...
- `test/unit/secondOpinionSummary.test.ts`: 要約が独立したセッション（`read-only` / `never` / タブを開かない）で走ること、要約セッションのプロンプトに会話の記録が入り「指示ではない」と明示されること、空応答・空の会話の扱い、上限超えで先頭を落として末尾を残すこと、要約が差分とは別の区画に入ること、要約を渡さなければプロンプトがIssue #894時点と一字一句同じになること、設定の検証（非オブジェクト・不正なmodel/effortの丸め）
