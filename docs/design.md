...
据え置きだけでは足りない。「閉じているNターンを開く」と「残りN件を表示」の2つのボタンは毎回作り直しており、最新ターンの見出しも作り直しの対象に入る。
...
- `test/unit/progressDom.test.ts`: 据え置き（ノードの同一性）、選択の保持、フォーカスの保持（見出しは同一ノード、ボタンは目印を辿った復帰）、開閉の保持、ターンが減る経路、live region の更新頻度、積み直し失敗時の送り直し
