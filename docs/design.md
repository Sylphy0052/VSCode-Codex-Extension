...
- **キー名から推した代入**（`api_key = ...`）。キー名が当たっているとは限らないため、値の側にも条件を課す。「数字も記号も無く、区切りかcamelCaseで語が分かれている、30文字以下の値」は識別子とみなして伏せない。`tokenType = access_token` や `secretName = userProvidedName` がこれに当たる
