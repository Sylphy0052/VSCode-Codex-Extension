...
測るのは**`start()` からのmonotonicな経過時間**で、`performance.now`（既定）を使う。差分しか使わないので基準時刻に意味は無く、NTP同期や手動の時刻変更で飛ばないことの方が重要である（`Date.now` は経過時間の計測には向かない）。**waitingReply・承認待ち・Evaluatorの応答待ちで止まっていた時間も、この予算に含む。**「エージェントが実際に処理していた時間だけを数える」方式（active execution budget）は採らない——停止の予測可能性を、実行時間としての正確さより優先する。
