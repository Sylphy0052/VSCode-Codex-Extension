...
- `done` の欠落、`prompt` の欠落
- `id` が `^[A-Za-z0-9_-]{1,50}$` に一致しない。この値はそのままパスとブランチ名に入るため、パストラバーサルやgitの引数解釈を防ぐために字種を絞る（`codex/argvBuilder.ts` がセッションidをUUIDで検証しているのと同じ理由）
- `maxParallel` が1未満、または10を超える。タスクの総数が50を超える
- テンプレート変数（§16.4）が未定義のタスクや、依存に挙げていないタスクを参照している
- `cwd` がワークスペースフォルダの外を指している（§16.16）
- Claudeタスクの `approvalMode` が承認要求を出さない値（`bypassPermissions` 相当）である。この設定では危険判定そのものが働かない（§16.7）
