...
| `/clear`   | `ChatViewManager.clearEntry`（会話を閉じて開き直す） | 実装済（#1264） |
...
### 擬似コマンド `/btw`（脇道の質問、issue #334、design.md §14.62）と `/clear`（issue #1264）

Claude Code側は原則「CLIが返す一覧をそのまま送ればよい」だが、`/btw`（脇道の質問）と`/clear`（会話のクリア）は例外。

`/btw`はCLIの90件の一覧に含まれておらず、`control_request`の`side_question` subtypeという別経路でしか呼べない（実測、CLI 2.1.235）。Codexの`/btw`（`CODEX_PSEUDO_COMMANDS`）と同じ仕組みで、拡張機能側が候補へ足す擬似コマンドとして実装した（`src/provider/pseudoCommands.ts`の`CLAUDE_PSEUDO_COMMANDS`）。

`/clear`は逆に、一覧には**含まれているのに効かない**。TUI層の機能でstream-json入力の経路には実体が無く、送っても`/clear`という文字列がただの発言としてモデルへ渡る（issue #1264）。候補に出るのに何も起きない状態を避けるため、拡張機能側の`clearEntry`（会話を閉じて同じ作業フォルダで開き直す。エディタ右上のクリアアイコンと同じ実処理）へ振り替える。

| コマンド | 呼ぶもの                           | 状態            |
| -------- | ---------------------------------- | --------------- |
| `/btw`   | `side_question`                    | 実装済（#334）  |
| `/clear` | `ClaudeChatViewManager.clearEntry` | 実装済（#1264） |
...
**Claude Codeの`side_question`は主会話の文脈を暗黙に共有する（実測で確定、design.md §14.62）。** `history`を明示的に渡さなくても、それまでの主会話の内容を踏まえて答える。一方でCLIのtranscriptには質問・応答が一切残らない（同じくdesign.md §14.62で実測済み）。この2つを合わせると、**「脇道の質問はログに残らない」が「主会話の内容は脇道の質問のモデルに見えている」という非対称**になる。質問を秘匿したい場合でも、主会話で既に話した内容が答えに滲み出うる点に注意する。

**「CLIのtranscriptに残らない」は拡張機能自身の「会話を書き出す」機能（README.mdの[会話を書き出す](../README.md#会話を書き出す)参照）には及ばない**（issue #340横断レビュー指摘）。`/btw`の質問・応答は画面には`kind:'sideQuestion'`という項目として残っており、書き出す（クリップボード・ファイル・生テキスト）と他の発言と同じくMarkdownへ含まれる。
